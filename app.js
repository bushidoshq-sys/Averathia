...
function begin(){let rb=$("#recall");if(rb){rb.disabled=false;rb.textContent="↩ Return Early"}if(h.waterCapacity<1){alert("You need at least one Waterskin.");return}let nw=4*mins/1440,nf=mins/1440,nl=mins*2/3;if(h.water<nw){alert("Not enough water.");return}if(h.rations<nf){alert("Not enough rations.");return}if(h.lightMinutes<nl){alert("Not enough light.");return}h.lightMinutes-=nl;let now=Date.now(),total=mins*60000,eventTarget=adventureEventTarget(mins),eventIntervalMs=Math.max(5000,total/eventTarget);h.lastAdventure=null;h.pendingEvent=null;ensureTrophies();let mission=createMission();h.trip={journal:[],adventureLog:[],mission,start:now,end:now+total,half:now+total/2,midBossDone:false,mode,risk,durationMinutes:mins,eventTarget,eventIntervalMs,resourceModel:"elapsed-v1",resourceAt:now,forcedReturnWater:false,nextEvent:now+Math.min(15000,Math.max(5000,total/(eventTarget+1)))};$("#departSetup").classList.add("hide");$("#travel").classList.remove("hide");$("#departedAt").textContent=clock(h.trip.start);$("#returnAt").textContent=clock(h.trip.end);$("#runner").innerHTML=spriteHTML(h.sex,h.avatar,h.className);$("#log").innerHTML="";addlog(`MISSION: ${mission.title} — ${mission.brief}`);renderAdventureLog();save();tick()}
