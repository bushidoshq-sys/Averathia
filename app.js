...
let sex="Male",avatar=0,cs=[],pick=null,h=null,tab="Weapons",inventoryTab="Inventory",mode="auto",risk="Normal",mins=1,customDurationActive=false,timer=null,autonomousCombatRunning=false,homeCoinMode="take",homeCoinDenom="gp",homeCoinAmount="";
...
function renderHome(){
 ...
 if(h.lastAdventure&&$("#lastAdventureText")){$("#lastAdventureText").textContent=h.lastAdventure;if($("#lastAdventureSummary"))$("#lastAdventureSummary").textContent="📖 "+(h.lastAdventureTitle||"Journey Log")}
...
function journeyNotificationsEnabled(){return localStorage.getItem("averathia-journey-notifications")==="1"}
async function setJourneyNotificationsEnabled(on){
 if(!on){localStorage.setItem("averathia-journey-notifications","0");return false}
 if(!("Notification" in window))return false;
 let permission=Notification.permission;if(permission!=="granted")permission=await Notification.requestPermission();
 let enabled=permission==="granted";localStorage.setItem("averathia-journey-notifications",enabled?"1":"0");return enabled
}
function journeyNotification(title,body){
 if(!journeyNotificationsEnabled()||!("Notification" in window)||Notification.permission!=="granted")return;
 try{if(navigator.serviceWorker?.ready)navigator.serviceWorker.ready.then(r=>r.showNotification(title,{body,tag:"averathia-journey",renotify:true})).catch(()=>new Notification(title,{body}));else new Notification(title,{body})}catch(e){}
}
...
function updateJourneyModeRiskUI(){
 let riskBlock=$("#journeyRiskBlock"),desc=$("#riskDescription");
 if(riskBlock)riskBlock.classList.toggle("hide",mode==="present");
 if(desc){desc.classList.toggle("hide",mode==="present"||risk==="Normal");desc.textContent=risk==="Cautious"?"Cautious: the character avoids unnecessary danger, retreats sooner, and uses healing potions at 65% HP or below.":risk==="Bold"?"Bold: the character accepts more danger, retreats later, and uses healing potions at 25% HP or below.":""}
}
$$("[data-mode]").forEach(b=>b.onclick=()=>{mode=b.dataset.mode;$$("[data-mode]").forEach(x=>x.classList.toggle("on",x===b));updateJourneyModeRiskUI();refresh()});
$$("[data-risk]").forEach(b=>b.onclick=()=>{risk=b.dataset.risk;$$("[data-risk]").forEach(x=>x.classList.toggle("on",x===b));updateJourneyModeRiskUI();refresh()});
updateJourneyModeRiskUI();
...
function begin(){let rb=$("#recall");if(rb){rb.disabled=false;rb.textContent="↩ Return Early"}if(!hasBackpack()){alert("You need a Backpack before beginning a Journey.");return}if(carriedBulkPoints()>maxCarryBP()+1e-9){alert(`You are carrying ${formatBP(carriedBulkPoints())}/${formatBP(maxCarryBP())} BP. Reduce your load before beginning a Journey.`);return}if(diseaseJourneyBlocked()){alert(`${journeyBlockingCondition()?.name||"Current condition"} prevents travel. You cannot begin a Journey.`);return}if(timedConditionJourneyBlocked()){alert(`${timedConditionBlockingName()} prevents travel. You cannot begin a Journey.`);return}let ret=retainerSelected(),mounts=plannedHorseCount(),horseAttempt=hasHouse()&&h.mounts.useOnJourney;if(horseAttempt&&!mounts){alert("Not enough Riding Horses for mounted travel with this party.");return}let needs=survivalNeedsForMinutes(mins),nw=needs.waterSkins,nf=needs.foodDays,nl=needs.activeATHours*60/AT_RATE;if(h.waterCapacity<nw){alert(`You need ${nw} Waterskins for this party.`);return}if(h.water+1e-9<nw){alert(`Fill ${nw} Waterskins before beginning the Journey.`);return}if(h.rations<nf){alert("Not enough rations.");return}if(mounts&&h.mounts.feedDays+1e-9<needs.mountFeedDays){alert("Not enough Mount Feed.");return}if(journeyUsableLightMinutes()<nl){alert("Not enough usable light. A Lantern is required to use lamp oil.");return}let journeyLight=consumeJourneyLight(nl);if(!journeyLight.ok){alert("Not enough usable light.");return}let now=Date.now(),total=mins*60000,eventTarget=adventureEventTarget(mins),eventIntervalMs=Math.max(5000,total/eventTarget);h.pendingEvent=null;ensureTrophies();let mission=createMission();h.trip={journal:[],adventureLog:[],mission,start:now,end:now+total,half:now+total/2,midBossDone:false,avTreasureXpCP:0,mode,risk,durationMinutes:mins,eventTarget,eventIntervalMs,resourceModel:"at-elapsed-v3-water-refill",resourceAt:now,waterRefillMode:"normal",waterStart:h.water,forcedReturnWater:false,retainer:ret,mountsUsed:mounts,mountedTravel:!!mounts,lightSource:journeyLight.source,plannedActiveATHours:needs.activeATHours,plannedSleepATHours:needs.sleepATHours,startHP:h.hp,startXP:h.xp,startWalletCP:walletCP(),completedSleeps:0,nextEvent:now+Math.min(15000,Math.max(5000,total/(eventTarget+1)))};$("#departSetup").classList.add("hide");$("#travel").classList.remove("hide");$("#departedAt").textContent=clock(h.trip.start);$("#returnAt").textContent=clock(h.trip.end);$("#runner").innerHTML=spriteHTML(h.sex,h.avatar,h.className);$("#log").innerHTML="";let mountText=mounts===1?" Mount secured.":mounts>1?" Mounts secured.":"";
 addlog(`${mission.title} — ${mission.brief} ${mins} min Journey · ${risk}${mountText}`,"Departure");
...
const JOURNEY_FLAVOR={"outdoor":["You notice a row of tiny stones placed at equal distances beside the path.","A bent copper coin has been pressed into the bark of a roadside tree.","The wind carries the smell of woodsmoke, but no smoke is visible.","A patch of grass has been flattened into a near-perfect circle.","A short piece of string lies beside the trail with three careful knots tied into it.","A weathered post bears a symbol you do not recognize.","Several black feathers are arranged in a rough circle beside the road.","You hear three knocks from somewhere beyond the path. They do not repeat.","A smooth white pebble sits on top of a weathered milestone.","Someone scratched a crooked arrow into a flat rock. It points nowhere useful.","You find the remains of a tiny fire, cold enough to have been abandoned long ago.","A strip of faded blue cloth is caught on a thorn bush.","For a moment you hear running water, although no stream is nearby.","An old boot print in dried mud is much larger than your own.","A little bundle of dry grass has been tied around a branch with red thread.","A line of mushrooms grows in an almost perfectly straight line.","A rusted key lies half buried in the dirt. There is no lock nearby.","A patch of ground beside the road is completely bare of plants.","A distant bell rings once, somewhere beyond sight.","Five flat stones have been stacked beside the trail, smallest on top.","A tiny carved wooden bird lies in the dust with one wing missing.","You pass a cairn with one freshly placed stone among the weathered ones.","A swarm of gnats hangs motionless above a ditch until you pass.","A single glove has been nailed to a fence post.","You find three nutshells placed neatly in a triangle.","A faded ribbon has been tied around a stone rather than a tree.","The road briefly smells of rotten straw before the scent vanishes.","A cluster of animal tracks circles a boulder without approaching it.","Someone has left a wooden spoon standing upright in the soil.","A shallow groove crosses the road from one ditch to the other.","A small patch of frost remains despite the mild air.","A cracked clay bead glints among the gravel.","You spot an old rope hanging from a high branch with nothing attached to it.","A tiny pile of salt has been left beneath a roadside stone.","A weathered plank beside the road has seven tally marks cut into it.","You find a snail shell painted with a faded red stripe.","A crow watches from a post until you pass, then flies the opposite way.","A small animal bone has been wrapped in clean linen.","You find a circle drawn in ash on a flat stone.","A handful of grain has been scattered in a straight line across the road."],"underground":["Shallow marks on the wall look almost like archaic writing.","A short piece of string with three knots lies in a dry crack in the floor.","Three pebbles have been placed in a neat line along a ledge.","A patch of stone smells faintly of rotten straw for no obvious reason.","You hear a soft scrape somewhere beyond the light, then silence.","A rusted key lies between two flagstones. There is no lock nearby.","Someone has drawn a small circle in soot on the wall.","A tiny carved wooden bird sits in a wall niche with one wing missing.","A line of old wax drips runs along the floor and stops abruptly.","Five flat chips of stone have been stacked on a ledge.","A narrow crack gives off a brief breath of unexpectedly warm air.","A patch of wall is noticeably colder than the stone around it.","A faded strip of cloth has been wedged into a mortar joint.","You find a single dry leaf far from any visible entrance.","A small pile of salt sits untouched in a corner.","A black feather lies on the floor without dust beneath it.","Several scratches on the floor form an arrow pointing at a blank wall.","A faint tapping echoes three times and does not return.","A clay bead has rolled into the corner of a step.","Someone has tied red thread around an iron ring in the wall.","One flagstone is polished smooth while those around it remain rough.","A tiny pile of clean sand sits in the middle of the passage.","An old boot print remains visible in a patch of dust.","A shallow cup carved into the floor contains clear water.","A row of seven tally marks has been cut into the stone.","A broken spoon lies carefully centered in the passage.","A smell like extinguished candles lingers for several steps.","A thin chalk line crosses the floor from wall to wall.","A small iron nail has been driven into the stone at shoulder height.","A coin-sized patch of wall has been rubbed completely smooth.","A brittle length of cord hangs from a ceiling hook.","A tiny bone has been wrapped in a scrap of linen and left on a ledge.","The passage briefly carries the sound of running water from somewhere below.","A little heap of grey ash sits where there is nothing obvious to burn.","A crude face has been scratched into the stone with only one eye.","A narrow groove in the floor ends at a wall with no opening.","A drop of dark wax has hardened on a clean patch of stone.","Four pebbles sit at the corners of an otherwise empty floor tile.","A small copper ring lies in the dust, bent nearly flat.","For several steps your footsteps seem to echo one beat late."],"indoor":["A short piece of string with three knots has been left on the floor.","Faint marks on a wall look almost like archaic writing.","A rusted key rests on a ledge with no obvious lock nearby.","A strip of faded blue cloth protrudes from beneath a piece of furniture.","Someone has drawn a tiny circle in soot near the floor.","Five small objects have been arranged from largest to smallest.","The room briefly smells of rotten straw, then the scent disappears.","A tiny carved wooden bird with one wing missing sits in a corner.","A single black feather lies on a surprisingly clean patch of floor.","Three soft knocks come from somewhere beyond the room and do not repeat.","A candle stub has been placed upright although it was never lit.","A little pile of salt sits beside a doorway.","Seven tally marks have been scratched into the underside of a sill.","A bent copper coin has been pushed into a crack in the wall.","A scrap of paper is folded many times but is completely blank.","A length of red thread has been tied around a door hinge.","One floorboard is polished much more than the others.","A dry leaf lies in the middle of the room.","A tiny clay bead is wedged between two stones.","A spoon has been placed carefully against the wall, bowl downward.","A small animal bone is wrapped in clean linen.","A thin chalk line runs across the threshold.","A faint smell of extinguished candles follows you into the next space.","A nail has been driven into a beam far above easy reach.","A little heap of ash sits on bare stone with no scorch mark beneath it.","A shallow scratch on the floor forms an arrow toward an empty corner.","A piece of cord hangs from a hook with nothing attached.","A single drop of dark wax has hardened on a tabletop.","A smooth white pebble rests in a wall niche.","Four buttons have been arranged in a square.","A cracked wooden cup has been turned upside down in the center of the floor.","A tiny bundle of straw is tied with red thread.","A patch of wall feels colder than the rest of the room.","A faint scraping sound stops whenever you stop walking.","A copper ring has been bent almost flat and left beside the doorway.","A small square of cloth has been pinned to the wall with a thorn.","Someone has marked a single X on the ceiling.","A line of dust has been swept aside in a perfectly straight path.","A small stack of blank paper has one page torn neatly in half.","For a moment you can smell rain despite being under a roof."],"generic":["You notice a short piece of string with three careful knots.","A faint smell of rotten straw appears and fades again.","A tiny carved wooden bird with one wing missing has been left behind.","A bent copper coin lies where someone could easily have picked it up.","Three small stones have been arranged in a straight line.","A rusted key has been discarded with no obvious lock nearby.","A strip of faded blue cloth catches your attention.","You hear three quiet knocks from somewhere out of sight.","A little bundle of dry material has been tied with red thread.","A small animal bone has been wrapped in clean linen.","A smooth white pebble has been placed somewhere conspicuous.","Seven tally marks have been scratched into a nearby surface.","A tiny pile of salt has been left undisturbed.","A cracked clay bead lies in plain sight.","A single black feather rests where it seems oddly deliberate.","Someone has drawn a small circle in ash.","A thin chalk line crosses your route.","A copper ring has been bent nearly flat.","A faint scraping noise stops as soon as you notice it.","For a moment the air feels noticeably colder, then returns to normal."],"town":["You pass a shuttered window with a candle burning behind it.","A chalk mark shaped like a hook has been drawn low on a wall.","Three copper buttons lie in a gutter in a perfect row.","A torn playing card is wedged beneath a doorstep.","Someone has tied a red thread around an iron drain ring.","A small pile of salt sits at the mouth of an alley.","A bent key has been left on a windowsill.","A cat watches you from a roof and disappears the moment you look away.","A scrap of cloth hangs from a sign bracket far above the street.","You hear a door close somewhere ahead, but the street remains empty.","A tiny wooden bird has been placed beneath a bench.","Seven tally marks have been scratched into a brick near ground level.","A single boot print crosses a dusty threshold and does not continue.","A cracked clay bead rolls from beneath a cart wheel.","A faded ribbon has been tied around a street post.","A narrow chalk line runs from one doorway to another.","A copper coin has been hammered nearly flat and pushed into a wall crack.","A smell of rotten straw drifts from an empty alley and quickly fades.","A little bundle of dry herbs lies untouched beside a doorway.","A black feather has been tucked carefully behind a loose brick.","Someone has stacked four broken roof tiles beside a drain.","A spoon lies in the middle of the street with its handle pointing north.","A bell rings once from somewhere nearby, then nothing follows.","A square of clean pavement stands out among the surrounding dust.","A piece of string with three knots hangs from a door latch.","A small animal bone wrapped in linen rests on a low wall.","A chalk circle surrounds a nail driven into a wooden post.","A shutter bears a symbol that has been carefully scratched out.","A handful of grain has been scattered across an otherwise clean doorstep.","A dry leaf spins in place despite the still air.","A tiny patch of frost clings to the shadow beneath a stair.","A blank scrap of paper has been pinned to a wall.","A rusted ring hangs from a hook where no sign remains.","Someone has placed a white pebble on each side of a doorway.","A faint scraping sound comes from behind a wall and stops abruptly.","A candle stub stands upright in a gutter without having melted.","A short row of footprints begins at a wall and ends at another.","A strip of blue cloth is caught beneath a closed door.","A wooden cup sits upside down on a step.","For several paces the street smells faintly of rain though the stones are dry."]};
const JOURNEY_MISSION_ENVIRONMENT={
 "Recover a lost arcane volume":"indoor","Seek a vanished scholar's manuscript":"generic","Investigate an abandoned magical library":"indoor",
 "Steal a guarded cache":"indoor","Find a legendary jewel":"town","Raid a forgotten strongroom":"indoor",
 "Answer a challenge of arms":"outdoor","Break a threat on the road":"outdoor","Win a deed worth remembering":"generic",
 "Recover a saint's relic":"indoor","Cleanse a desecrated resting place":"underground","Seek a lost reliquary":"indoor",
 "Recover an ancestral rune":"underground","Trace a lost family inscription":"underground","Search a forgotten deep hall":"underground",
 "Protect the woodland":"outdoor","Seek a rare seed":"outdoor","Preserve a vanishing grove":"outdoor"
};
...
function maybeJourneyFlavor(){
 ...
 let env=journeyFlavorEnvironment(),pool=JOURNEY_FLAVOR[env]||JOURNEY_FLAVOR.generic,last=h.trip.lastFlavorText||"",choices=pool.filter(x=>x!==last),t=choices[d(choices.length)-1]||pool[0];
