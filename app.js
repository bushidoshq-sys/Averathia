...
$("#chooseCharacter").onclick=()=>{if(!cs[0])return;let c=structuredClone(cs[0]);h={...c,maxhp:c.hp,xp:0,level:1,name:$("#charName").value.trim()||fullName(),sex,avatar,className:chosenClass,mechanicsClass:chosenClass,gp:0,sp:0,cp:0,inv:[],rations:0,water:0,waterCapacity:0,lightStock:{torchMinutes:0,oilMinutes:0,legacyMinutes:0},lightMinutes:0,buffAgeAt:Date.now(),diseases:[],trollWeaknessKnown:false,timedConditions:[],timedConditionAgeAt:Date.now()};h.gp=c.gold;addClassStartingItems();$("#create").classList.add("hide");$("#game").classList.remove("hide");save();home();page("town")};
const CLASS_LEVELS={
...
function contractCentipedeSickness(e){
 ...
 let until=averathiaNow()+10*86400000;addDisease({type:"centipede",name:"Centipede Sickness",source:e?.n||"Giant Centipede",activityBlockedUntilAT:until,autoRecoverAtAT:until});
 clog("Centipede Sickness contracted — physical activity is blocked for 10 Averathia days. The Journey must end after this encounter.");return true
}
function updateRest(){
...
function begin(){let rb=$("#recall");if(rb){rb.disabled=false;rb.textContent="↩ Return Early"}if(diseaseJourneyBlocked()){alert(`${journeyBlockingCondition()?.name||"Current condition"} prevents travel. You cannot begin a Journey.`);return}if(timedConditionJourneyBlocked()){alert(`${timedConditionBlockingName()} prevents travel. You cannot begin a Journey.`);return}if(h.waterCapacity<1){alert("You need at least one Waterskin.");return}let needs=survivalNeedsForMinutes(mins),nw=needs.waterSkins,nf=needs.foodDays,nl=mins*2/3;if(h.water<nw){alert("Not enough water.");return}if(h.rations<nf){alert("Not enough rations.");return}if(totalLightMinutes()<nl){alert("Not enough light.");return}consumeLightMinutes(nl);let now=Date.now(),total=mins*60000,eventTarget=adventureEventTarget(mins),eventIntervalMs=Math.max(5000,total/eventTarget);h.lastAdventure=null;h.pendingEvent=null;ensureTrophies();let mission=createMission();h.trip={journal:[],adventureLog:[],mission,start:now,end:now+total,half:now+total/2,midBossDone:false,rcTreasureXpCP:0,mode,risk,durationMinutes:mins,eventTarget,eventIntervalMs,resourceModel:"at-elapsed-v2",resourceAt:now,forcedReturnWater:false,nextEvent:now+Math.min(15000,Math.max(5000,total/(eventTarget+1)))};$("#departSetup").classList.add("hide");$("#travel").classList.remove("hide");$("#departedAt").textContent=clock(h.trip.start);$("#returnAt").textContent=clock(h.trip.end);$("#runner").innerHTML=spriteHTML(h.sex,h.avatar,h.className);$("#log").innerHTML="";addlog(`MISSION: ${mission.title} — ${mission.brief}`);renderAdventureLog();save();tick()}
...
function enemyStrike(filter=null,tickRegen=true){
 for(const e of living().filter(x=>!filter||filter(x))){
  ...
  for(const a of attacks){
   let r=d(20),blindPenalty=e.blindRounds>0?-6:0,stingBonus=(a.sting&&clawHits>0)?2:0,ferocityBonus=e.ferocityAttackBonus&&playerIsHumanlike()?Number(e.ferocityAttackBonus):0,conditionBonus=enemyTimedAttackBonus(),attackBonus=stingBonus+ferocityBonus+conditionBonus,need=Math.max(2,(20-monsterHitModifier(e))-effectiveAC(!!a.ranged));
   ...
   clog(`${e.n} hits with ${a.name} for ${dmg}${r===20?" — critical":""}${stingBonus?` (+${stingBonus} sting attack)`:""}${ferocityBonus?` (+${ferocityBonus} ferocity)`:""}${conditionBonus?` (+${conditionBonus} vs dancing target)`:""}.`);
   if(a.claw)clawHits++;
