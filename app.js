...
function renderAv(){$("#avatars").innerHTML=[0,1,2].map((a,i)=>`<button class="avatarBtn ${i===avatar?"on":""} ${chosenClass==="Arcanist"&&sex==="Male"&&i===0?"arcanistMale1":""}" data-av="${i}">${chibiHTML(sex,i)}</button>`).join("");$$("[data-av]").forEach(b=>b.onclick=()=>{avatar=+b.dataset.av;renderAv()});$("#bigAvatar").innerHTML=chibiHTML(sex,avatar,true);$("#previewLabel").textContent=(($("#charName").value||"").trim()||"Selected Character")+" — "+chosenClass}
