...
function journeyFlavorEnvironment(){
 let title=h?.trip?.mission?.title||"",cls=h?.className||"";
 if(cls==="Dwarf"||/deep hall|mine|ancestral rune/i.test(title))return"underground";
 if(cls==="Arcanist"||/archive|library|strongroom|reliquary|relic|resting place/i.test(title))return"indoor";
 if(cls==="Elf"||/road|woodland|seed|grove/i.test(title))return"outdoor";
 return"generic"
}
