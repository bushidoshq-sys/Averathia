...
function enemyStrike(filter=null,tickRegen=true){
 for(const e of living().filter(x=>!filter||filter(x))){
  ...
  for(const a of attacks){
   let r=d(20),blindPenalty=e.blindRounds>0?-6:0,stingBonus=(a.sting&&clawHits>0)?2:0,ferocityBonus=e.ferocityAttackBonus&&playerIsHumanlike()?Number(e.ferocityAttackBonus):0,attackBonus=stingBonus+ferocityBonus,need=Math.max(2,(20-monsterHitModifier(e))-effectiveAC(!!a.ranged));
   ...
   clog(`${e.n} hits with ${a.name} for ${dmg}${r===20?" — critical":""}${stingBonus?` (+${stingBonus} sting attack)`:""}${ferocityBonus?` (+${ferocityBonus} ferocity)`:""}.`);
   if(a.claw)clawHits++;
...
function resolveAttack(){
 ...
 if(!h.combat)return;if(!living().length)return finishCombat();tickSpellBuffs();tickEnemySpellEffects();tickPlayerConditions();c.round++;save();renderCombat()
}
