...
function castCombatSpell(id,holdMode=null){
 let s=(SPELLS[h.className]||[]).find(x=>x.id===id),c=h.combat;if(!s||!c||!availableCombatSpells().some(x=>x.id===id))return;if(c.paralyzed){clog(`${h.name} is paralyzed and cannot cast.`);return renderCombat()}
...
function tickPlayerConditions(){if(!h?.combat)return;if(h.combat.paralyzed){if(!Number.isFinite(h.combat.paralyzedRounds))h.combat.paralyzedRounds=1;h.combat.paralyzedRounds--;if(h.combat.paralyzedRounds<=0){h.combat.paralyzed=false;h.combat.paralyzedRounds=0;clog(`${h.name} can move again.`)}}}
