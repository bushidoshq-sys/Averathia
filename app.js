...
function playerStrikeSingle(){
 ...
 let r=d(20),isRanged=RANGED_WEAPONS.has(w),atkMod=isRanged?mod(h.stats.DEX):mod(h.stats.STR),dmgMod=isRanged?0:mod(h.stats.STR);
...
function changeRange(){
 ...
 if(i>0){h.combat.range=order[i-1];clog(`You close the distance: ${h.combat.range}.`);enemyStrike()}
 else{h.combat.range="Close";clog("You break out to Close range.");enemyStrike()}
 if(h.combat){h.combat.round++;save();renderCombat()}
}
...
function usePotionCombat(){let i=h.inv.findIndex(x=>x.n==="Healing Potion");if(i<0){clog("No Healing Potion.");return renderCombat()}let heal=d(6)+1;h.hp=Math.min(h.maxhp,h.hp+heal);h.inv.splice(i,1);clog(`Potion restores ${heal} HP.`);enemyStrike();if(h.combat){h.combat.round++;save();renderCombat()}}
function retreatCombat(){if(d(6)>=3){addlog("You escape the encounter.");h.combat=null;endTripPause();save();page("depart");refresh();tick()}else{clog("Retreat fails.");enemyStrike();if(h.combat){h.combat.round++;save();renderCombat()}}}
function renderCombat(){if(!h?.combat)return;page("combat");let c=h.combat,cs=combatStats();$("#combatRound").textContent=`Round ${c.round}`;$("#combatStatus").innerHTML=`❤️ HP ${h.hp}/${h.maxhp} · 🛡 AC ${cs.ac} · ⚔ ${cs.weapon} (${cs.damage}) · 📏 ${c.range||"Close"}`;$("#combatEnemies").innerHTML=c.enemies.map(e=>`<button class="enemyCard ${c.target===e.id?"target":""}" data-target="${e.id}" ${e.hp<=0?"disabled":""}><b>${e.boss?"👑 BOSS — ":""}${e.n}</b> · HP ${e.hp}/${e.maxhp} · AC ${e.ac}</button>`).join("");$("#combatLog").innerHTML=c.log.map(x=>`<div>${x}</div>`).join("");$("#combatLog").scrollTop=$("#combatLog").scrollHeight;$("[data-target]").forEach(b=>b.onclick=()=>{c.target=+b.dataset.target;save();renderCombat()});renderSpellButton()}
...
$("#attackBtn").onclick=resolveAttack;$("#rangeBtn").onclick=changeRange;$("#potionBtn").onclick=usePotionCombat;$("#retreatBtn").onclick=retreatCombat;
