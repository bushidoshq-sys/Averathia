...
function journeyCompletedSleeps(now=Date.now(),trip=h?.trip){
 ...
 return Math.max(0,Math.floor(atMinutes/(24*60)))
}
function journeyInSleepWindow(now=Date.now(),trip=h?.trip){
 if(!trip)return false;
 let atMinutes=journeyElapsedATMinutes(now,trip),within=atMinutes%(24*60);
 return within>=16*60
}
