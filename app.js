...
function survivalNeedsForMinutes(minutes){let days=atDaysForRTMinutes(minutes),ret=retainerSelected();return{foodDays:days*(ret?2:1),waterSkins:4*days,mountFeedDays:days*plannedHorseCount()}}
function mountedTravelReady(obj=h,minutes=mins){let used=plannedHorseCount(obj);return !!(used&&(obj.mounts.feedDays||0)+1e-9>=atDaysForRTMinutes(minutes)*used)}
...
function renderHome(){
 ...
 if(hasRoom()){
  ...
  let modeButtons='<div class="homeCoinToggle"><button data-home-coin-mode="store" class="'+(homeCoinMode==="store"?"on":"")+'">Store</button><button data-home-coin-mode="take" class="'+(homeCoinMode==="take"?"on":"")+'">Take</button></div>';
  let denomButtons='<div class="homeCoinToggle">'+["gp","sp","cp"].map(k=>'<button data-home-coin-denom="'+k+'" class="'+(homeCoinDenom===k?"on":"")+'">'+k.toUpperCase()+'</button>').join("")+'</div>';
...
function consumeTripSurvivalResources(now=Date.now()){
 ...
 if(dt>0){ageTimedBuffsClock(t);let days=scale*dt/86400000;h.rations=Math.max(0,(h.rations||0)-days*(h.trip.retainer?2:1));h.water=Math.max(0,(h.water||0)-4*days);if(h.trip.mountsUsed)h.mounts.feedDays=Math.max(0,(h.mounts.feedDays||0)-days*h.trip.mountsUsed);h.trip.resourceAt=t}
...
function rcApplyTreasure(t){
 ...
 if(h.trip&&(left.length||credited.leftCP)){let bits=[];if(left.length)bits.push(`${left.length} item${left.length===1?"":"s"}`);if(credited.leftCP)bits.push(coinTextCP(credited.leftCP)+" in coins");addlog(`Carry limit: left behind ${bits.join(" and ")}.`)}
