...
function usePotionCombat(){if(h?.combat?.paralyzed){clog(`${h.name} is paralyzed and cannot use a potion.`);return renderCombat()}let i=h.inv.findIndex(x=>x.n==="Healing Potion");if(i<0){clog("No Healing Potion.");return renderCombat()}if(mummyDiseaseActive()){clog("Mummy disease prevents the Healing Potion from restoring HP.");return renderCombat()}let heal=d(6)+1;h.hp=Math.min(h.maxhp,h.hp+heal);h.inv.splice(i,1);clog(`Potion restores ${heal} HP.`);enemyStrike();if(h.combat){tickSpellBuffs();tickEnemySpellEffects();tickPlayerConditions();h.combat.round++;save();renderCombat()}}
