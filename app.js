...
function renderClasses(){$("#classList").innerHTML=CLASSES.map(c=>`<button class="classChoice ${c[0]===chosenClass?"on":""}" data-class="${c[0]}"><b>${c[0]}</b></button>`).join("");$$("[data-class]").forEach(b=>b.onclick=()=>{chosenClass=b.dataset.class;renderClasses();renderAv();renderCandidates()})}
...
function addlog(t){$("#log").insertAdjacentHTML("beforeend",`<p>${t}<span style="float:right;color:#aaa">${clock(Date.now())}</span></p>`)}
...
function monsterNeed(e){return (19-Math.floor(Math.max(0,e.hdDice-1)/2))-(combatStats().ac+spellACBonus())}
function clog(s){h.combat.log.push(s);if(h.combat.log.length>40)h.combat.log.shift()}
...
function makeCombat(isBoss=false){
 ...
 h.combat={round:1,enemies:en,target:0,log:[],skipNext:false,isBoss,rcTPL:tpl,rcIAHD:total,rcChallengePct:pct,rcChallenge:rcChallengeName(pct)};
...
function startUndeadCombat(en,opening){
 ...
 h.combat={round:1,enemies:en,target:0,log:[],skipNext:false,isBoss:false};
...
function resolveTurnUndead(ev){
 ...
 else{addlog("All undead are driven off. No combat remains.");save();page("depart");refresh()}
}
...
function enemyStrike(){
 for(const e of living()){
  ...
  if(r===20||r>=monsterNeed(e)){
   ...
   clog(`${e.n} hits for ${dmg}${r===20?" — critical":""}.`);
   if(e.special==="poison"){
...
function resolveAttack(){let c=h.combat,pr=d(6),er=d(6);while(pr===er){pr=d(6);er=d(6)}clog(`Initiative: you ${pr}, enemies ${er}.`);if(pr>er){playerStrike();if(living().length)enemyStrike()}else if(enemyStrike())playerStrike();if(!h.combat)return;if(!living().length)return finishCombat();tickSpellBuffs();tickEnemySpellEffects();c.round++;save();renderCombat()}
function finishCombat(){
 ...
 h.combat=null;save();page("depart");refresh()
}
...
function applyEventChoice(ev,ch){
 ...
 if(ch?.result==="clericWis"){
   ...
   h.pendingEvent=null;save();renderPendingEvent();return
 }
 ...
 save();h.pendingEvent=null;renderPendingEvent()
}
function event(){
 if(!h||!h.trip||h.combat||h.pendingEvent)return;
 let ev=pickEvent();
 if(ev.type==="Encounter"){
   if(h.trip.mode==="present"){h.pendingEvent=ev;renderPendingEvent();save();return}
 ...
 if(ev.choices.length&&h.trip.mode==="present"){h.pendingEvent=ev;renderPendingEvent();save();return}
