...
function gainLevel(){
 ...
 else{let gain=["Fighter","Dwarf"].includes(h.className)?2:1;h.maxhp+=gain;h.hp+=gain;h.lastLevelGain={level:h.level,hp:gain,roll:null,con:0}}
...
function updateRest(){
 if(h?.restUntil&&Date.now()>=h.restUntil){
   h.restUntil=null;h.hp=Math.min(h.maxhp,h.hp+Math.ceil(h.maxhp*.25));resetDailySpells();save();return true
 }return false
}
...
function combatStats(){let weapon=activeWeapon(),armor=h.inv.find(x=>x.kind==="armor"&&x.eq),shield=h.inv.find(x=>x.kind==="shield"&&x.eq),wd=weapon?itemData(weapon.n):{},ad=armor?itemData(armor.n):{};return{weapon:weapon?.n||"Unarmed",damage:wd.damage||"1d2",armor:armor?.n||"None",shield:shield?.n||"None",ac:(ad.ac??9)+(shield?-1:0)}}
...
function endTripPause(){if(!h?.trip?.pauseStart)return;let dt=Date.now()-h.trip.pauseStart;for(const k of ["start","end","half","nextEvent"])if(Number.isFinite(h.trip[k]))h.trip[k]+=dt;h.trip.pauseStart=null}
function begin(){let rb=$("#recall");if(rb)rb.disabled=false;if(h.waterCapacity<1){alert("You need at least one Waterskin.");return}let nw=4*mins/1440,nf=mins/1440,nl=mins*2/3;if(h.water<nw){alert("Not enough water.");return}if(h.rations<nf){alert("Not enough rations.");return}if(h.lightMinutes<nl){alert("Not enough light.");return}h.rations-=nf;h.water-=nw;h.lightMinutes-=nl;let now=Date.now();h.lastAdventure=null;h.pendingEvent=null;ensureTrophies();let mission=createMission();h.trip={journal:[],adventureLog:[],mission,start:now,end:now+mins*60000,half:now+mins*30000,midBossDone:false,mode,risk,nextEvent:now+Math.min(15000,Math.max(5000,mins*60000/(Math.max(1,{1:1,5:5,10:5,30:6,60:6,120:8,480:8,1440:10}[mins])+1)))};$("#departSetup").classList.add("hide");$("#travel").classList.remove("hide");$("#departedAt").textContent=clock(h.trip.start);$("#returnAt").textContent=clock(h.trip.end);$("#runner").innerHTML=spriteHTML(h.sex,h.avatar,h.className);$("#log").innerHTML="";addlog(`MISSION: ${mission.title} — ${mission.brief}`);renderAdventureLog();save();tick()}
function tick(){clearTimeout(timer);if(!h.trip)return;let now=h.trip.pauseStart||Date.now(),total=h.trip.end-h.trip.start,elapsed=Math.max(0,now-h.trip.start),pct=Math.min(1,elapsed/total),outbound=pct<=0.5,runnerPct=outbound?pct*200:(1-pct)*200;$("#fill").style.width="0%";$("#runner").style.left=runnerPct+"%";$("#runner").style.transform=outbound?"translate(-50%,-62%) scaleX(-1)":"translate(-50%,-62%) scaleX(1)";$("#phase").textContent=(outbound?"OUTBOUND / ADVENTURING":"RETURNING")+(h.trip.mission?` · ${h.trip.mission.title}`:"");let remaining=Math.max(0,h.trip.end-now);$("#remainingClock").textContent=`${Math.floor(remaining/60000)}:${String(Math.floor(remaining/1000)%60).padStart(2,"0")}`;$("#returnAt").textContent=clock(h.trip.end);$("#timeText").textContent=`Elapsed ${Math.floor(elapsed/60000)}:${String(Math.floor(elapsed/1000)%60).padStart(2,"0")} · Remaining ${Math.floor(remaining/60000)}:${String(Math.floor(remaining/1000)%60).padStart(2,"0")}`;if(now>=h.trip.half&&!h.trip.midBossDone&&!h.combat&&!h.pendingEvent){h.trip.midBossDone=true;save();makeCombat(true)}if(now>=h.trip.nextEvent&&now<h.trip.end&&!h.combat){event();h.trip.nextEvent+=Math.max(5000,total/Math.max(1,{1:1,5:5,10:5,30:6,60:6,120:8,480:8,1440:10}[mins]));save()}if(now>=h.trip.end){addlog("Returned to town.");h.trip=null;$("#travel").classList.add("hide");$("#departSetup").classList.remove("hide");home();page("town");return}timer=setTimeout(tick,500)}
...
const MONSTERS=[{"id":"kobold","n":"Kobold","ac":7,"hdDice":1,"hdAdj":-1,"damage":["1d4"],"meleeDamage":"1d4","rangedDamage":"1d4","meleeWeapon":"Dagger","rangedWeapon":"Sling","xp":5,"source":"RC-adapted","humanoid":true},{"id":"goblin","n":"Goblin","ac":6,"hdDice":1,"hdAdj":-1,"damage":["1d6"],"meleeDamage":"1d6","rangedDamage":"1d6","meleeWeapon":"Short Sword","rangedWeapon":"Short Bow","xp":5,"source":"RC-adapted","humanoid":true},{"id":"orc","n":"Orc","ac":6,"hdDice":1,"hdAdj":0,"damage":["1d6"],"meleeDamage":"1d6","rangedDamage":"1d6","meleeWeapon":"Spear","rangedWeapon":"Short Bow","xp":10,"source":"RC-adapted","humanoid":true},{"id":"hobgoblin","n":"Hobgoblin","ac":6,"hdDice":1,"hdAdj":1,"damage":["1d8"],"meleeDamage":"1d8","rangedDamage":"1d6","meleeWeapon":"Sword","rangedWeapon":"Short Bow","xp":15,"source":"RC-adapted","humanoid":true},{"id":"gnoll","n":"Gnoll","ac":5,"hdDice":2,"hdAdj":0,"damage":["1d8"],"meleeDamage":"1d8","rangedDamage":"1d6","meleeWeapon":"Battle Axe","rangedWeapon":"Long Bow","xp":20,"source":"RC-adapted","humanoid":true},{"id":"skeleton","n":"Skeleton","ac":7,"hdDice":1,"hdAdj":0,"damage":["1d6"],"xp":10,"undead":true,"source":"RC-adapted","humanoid":false},{"id":"zombie","n":"Zombie","ac":8,"hdDice":2,"hdAdj":0,"damage":["1d8"],"xp":20,"undead":true,"slow":true,"source":"RC-adapted","humanoid":false},{"id":"ghoul","n":"Ghoul","ac":6,"hdDice":2,"hdAdj":0,"damage":["1d3"],"xp":25,"undead":true,"special":"paralysis","rcPowerBonuses":1,"rcAsterisks":1,"source":"RC-adapted","humanoid":false},{"id":"giant_rat","n":"Giant Rat","ac":7,"hdDice":1,"hdAdj":-1,"damage":["1d3"],"xp":5,"special":"disease","rcPowerBonuses":1,"rcAsterisks":1,"source":"RC-adapted","humanoid":false},{"id":"wolf","n":"Wolf","ac":7,"hdDice":2,"hdAdj":2,"damage":["1d6"],"xp":25,"source":"RC-adapted","humanoid":false},{"id":"dire_wolf","n":"Dire Wolf","ac":6,"hdDice":4,"hdAdj":1,"damage":["2d4"],"xp":75,"source":"RC-adapted","humanoid":false},{"id":"bear","n":"Bear","ac":6,"hdDice":4,"hdAdj":0,"damage":["1d8"],"xp":75,"source":"RC-adapted","humanoid":false},{"id":"giant_spider","n":"Giant Spider","ac":6,"hdDice":2,"hdAdj":0,"damage":["1d8"],"xp":35,"special":"poison","rcPowerBonuses":1,"rcAsterisks":1,"source":"RC-adapted","humanoid":false},{"id":"giant_centipede","n":"Giant Centipede","ac":9,"hdDice":1,"hdAdj":-1,"damage":["1"],"xp":6,"special":"poison","rcPowerBonuses":1,"rcAsterisks":1,"source":"RC-adapted","humanoid":false},{"id":"giant_scorpion","n":"Giant Scorpion","ac":2,"hdDice":4,"hdAdj":0,"damage":["1d10"],"xp":125,"special":"poison","rcPowerBonuses":1,"rcAsterisks":1,"source":"RC-adapted","humanoid":false},{"id":"giant_snake","n":"Giant Snake","ac":6,"hdDice":3,"hdAdj":0,"damage":["1d8"],"xp":50,"special":"poison","rcPowerBonuses":1,"rcAsterisks":1,"source":"RC-adapted","humanoid":false},{"id":"lizard_man","n":"Lizard Man","ac":5,"hdDice":2,"hdAdj":1,"damage":["1d6"],"meleeDamage":"1d6","rangedDamage":"1d6","meleeWeapon":"Spear","rangedWeapon":"Javelin","xp":25,"source":"RC-adapted","humanoid":true},{"id":"bandit","n":"Bandit","ac":6,"hdDice":1,"hdAdj":0,"damage":["1d6"],"meleeDamage":"1d6","rangedDamage":"1d6","meleeWeapon":"Short Sword","rangedWeapon":"Short Bow","xp":10,"source":"RC-adapted","humanoid":true},{"id":"brigand","n":"Brigand","ac":6,"hdDice":1,"hdAdj":0,"damage":["1d6"],"meleeDamage":"1d6","rangedDamage":"1d6","meleeWeapon":"Sword","rangedWeapon":"Light Crossbow","xp":10,"source":"RC-adapted","humanoid":true},{"id":"berserker","n":"Berserker","ac":7,"hdDice":1,"hdAdj":1,"damage":["1d8"],"meleeDamage":"1d8","meleeWeapon":"Battle Axe","xp":15,"source":"RC-adapted","humanoid":true},{"id":"ogre","n":"Ogre","ac":5,"hdDice":4,"hdAdj":1,"damage":["1d10"],"xp":125,"source":"RC-adapted","humanoid":true},{"id":"troll","n":"Troll","ac":4,"hdDice":6,"hdAdj":3,"damage":["1d8"],"xp":650,"special":"regeneration","rcPowerBonuses":1,"rcAsterisks":1,"source":"RC-adapted","humanoid":false},{"id":"minotaur","n":"Minotaur","ac":6,"hdDice":6,"hdAdj":0,"damage":["1d10"],"xp":275,"source":"RC-adapted","humanoid":false},{"id":"mummy","n":"Mummy","ac":3,"hdDice":5,"hdAdj":1,"damage":["1d12"],"xp":575,"undead":true,"special":"disease","rcPowerBonuses":1,"rcAsterisks":1,"source":"RC-adapted","humanoid":false}];
function rollExpr(x){let m=/(\d+)d(\d+)([+-]\d+)?/i.exec(x);if(!m)return 0,v=0;let n=+m[1];while(n--)v+=d(+m[2]);return Math.max(0,v+(+(m[3]||0)))}
...
const UNDEAD={
 skeleton:{id:"skeleton",n:"Skeleton",ac:7,hdDice:1,hdAdj:0,damage:"1d6",xp:10,undead:true,turnType:"Skeleton"},
 zombie:{id:"zombie",n:"Zombie",ac:8,hdDice:2,hdAdj:0,damage:"1d8",xp:20,undead:true,turnType:"Zombie"},
 ghoul:{id:"ghoul",n:"Ghoul",ac:6,hdDice:2,hdAdj:0,damage:"1d3",xp:25,undead:true,turnType:"Ghoul"}
};
...
function autonomousCombat(isBoss=false){
 ...
 while(h.combat&&living().length&&h.hp>0&&guard++<100){
   ...
   if(ratio<=autoPotionThreshold()){
     ...
     if(pi>=0){h.inv.splice(pi,1);let heal=d(6)+1;h.hp=Math.min(h.maxhp,h.hp+heal);clog(`Autonomous: potion restores ${heal} HP.`)}
   }
   ...
   tickSpellBuffs();tickEnemySpellEffects();if(h.combat)h.combat.round++;
 }
...
function playerStrikeSingle(){
 ...
 let w=combatStats().weapon;if(t.sleeping&&EDGED_WEAPONS.has(w)){let dmg=t.hp;t.hp=0;t.sleeping=false;t.disabledRounds=0;clog(`Sleeping ${t.n} is slain with a single edged-weapon blow (${dmg} HP).`);h.xp+=t.xp;clog(`${t.n} defeated. +${t.xp} XP.`);checkLevelUps();return}let at=ammoTypeFor(w);if(at&&!spendAmmoFor(w)){clog(`No ${at.toLowerCase()} left for ${w}.`);return}
 if(c.skipNext){clog("Critical fumble: you lose this initiative.");c.skipNext=false;return}
 let r=d(20),isRanged=RANGED_WEAPONS.has(w),atkMod=isRanged?mod(h.stats.DEX):mod(h.stats.STR),dmgMod=isRanged?0:mod(h.stats.STR);if(isRanged&&c.range==="Hand-to-Hand"&&(t.disabledRounds||0)<=0){clog(`${w} cannot be used effectively at Hand-to-Hand against a mobile target.`);return}
 if(r===1){clog("Natural 1 — critical fumble. Next initiative is lost.");c.skipNext=true}
 else if(r===20||r+atkMod+spellAttackBonus()+(isRanged?rangeAttackMod():0)>=characterNeed(t.ac)){
  ...
  let dmg=Math.max(1,rollExpr(combatStats().damage)+dmgMod+extra+flat);if(r===20)dmg*=2;t.hp=Math.max(0,t.hp-dmg);
  clog(`${r===20?"Critical hit! ":""}You hit ${t.n} for ${dmg}.`);
...
function monsterRangeStep(e){
 ...
 if(e.rangedDamage&&!e.meleeDamage&&i<3){h.combat.range=order[i+1];clog(`${e.n} opens the distance to ${h.combat.range}.`);return true}
 return false
}
 ...
 for(const e of living()){
  ...
  let mr=h.combat?.range||"Close",useRanged=false;if(e.rangedDamage&&mr!=="Hand-to-Hand"&&(e.ammo||0)>0){e.damage=e.rangedDamage;e.activeWeapon=e.rangedWeapon||"Ranged weapon";e.ammo--;useRanged=true}else if(e.meleeDamage){e.damage=e.meleeDamage;e.activeWeapon=e.meleeWeapon||"Melee weapon"}let need=Math.max(2,(20-monsterHitModifier(e))-effectiveAC(useRanged));if(r===20||r>=need){
   ...
   if(e.special==="paralysis"){
    ...
    if(!s.success){h.combat.paralyzed=true;clog(`${h.name} is paralyzed and cannot act.`)}}
   }
   if(e.special==="disease"){
    let s=savingThrow("Death/Poison");
...
function renderRangeButton(){
 ...
 let order=["Hand-to-Hand","Close","Medium","Long"],i=order.indexOf(h.combat.range||"Close"),haste=h.spells?.buffs?.some(x=>x.extraAttack);
 b.textContent=haste?"📏 Change Range ⚡":"📏 Change Range";
 b.onclick=()=>{i=order.indexOf(h.combat.range||"Close");$("#spellMenu")?.classList.add("hide");let choices=[];if(i>0)choices.push('<button data-range-dir="closer">⬅ Closer</button>');if(i<order.length-1)choices.push('<button data-range-dir="farther">Farther ➡</button>');if(haste)choices.push('<span class="small">Quickening: move up to 2 range bands</span>');menu.innerHTML=choices.join("");menu.classList.toggle("hide");$("[data-range-dir]").forEach(x=>x.onclick=()=>changeRange(x.dataset.rangeDir))}
}
function changeRange(direction){
 ...
 let order=["Hand-to-Hand","Close","Medium","Long"],i=order.indexOf(h.combat.range||"Close"),haste=h.spells?.buffs?.some(x=>x.extraAttack),steps=haste?2:1,ni=direction==="farther"?Math.min(order.length-1,i+steps):Math.max(0,i-steps);
 if(ni===i){clog(`You are already at ${order[i]} range.`);return renderCombat()}
 h.combat.range=order[ni];$("#rangeMenu")?.classList.add("hide");clog(`You move ${direction==="farther"?"farther away":"closer"}${haste?" under Quickening":""}: ${h.combat.range}.`);enemyStrike();
 if(h.combat){tickSpellBuffs();tickEnemySpellEffects();h.combat.round++;save();renderCombat()}
}
...
function finishCombat(){
 ...
 h.combat=null;endTripPause();save();page("depart");refresh();tick()
}
function usePotionCombat(){let i=h.inv.findIndex(x=>x.n==="Healing Potion");if(i<0){clog("No Healing Potion.");return renderCombat()}let heal=d(6)+1;h.hp=Math.min(h.maxhp,h.hp+heal);h.inv.splice(i,1);clog(`Potion restores ${heal} HP.`);enemyStrike();if(h.combat){tickSpellBuffs();tickEnemySpellEffects();h.combat.round++;save();renderCombat()}}
function retreatCombat(){if(d(6)>=3){addlog("You escape the encounter.");h.combat=null;endTripPause();save();page("depart");refresh();tick()}else{clog("Retreat fails.");enemyStrike();if(h.combat){tickSpellBuffs();tickEnemySpellEffects();h.combat.round++;save();renderCombat()}}}
function renderCombat(){if(!h?.combat)return;page("combat");let c=h.combat,cs=combatStats();$("#combatRound").textContent=`Round ${c.round}`;$("#combatStatus").innerHTML=`❤️ HP ${h.hp}/${h.maxhp} · 🛡 AC ${cs.ac} · ⚔ ${cs.weapon} (${cs.damage}) · 📏 ${c.range||"Close"}`;$("#combatEnemies").innerHTML=c.enemies.map(e=>`<button class="enemyCard ${c.target===e.id?"target":""}" data-target="${e.id}" ${e.hp<=0?"disabled":""}><b>${e.boss?"👑 BOSS — ":""}${e.n}</b> · HP ${e.hp}/${e.maxhp} · AC ${e.ac}</button>`).join("");$("#combatLog").innerHTML=c.log.map(x=>`<div>${x}</div>`).join("");$("#combatLog").scrollTop=$("#combatLog").scrollHeight;$("[data-target]").forEach(b=>b.onclick=()=>{c.target=+b.dataset.target;save();renderCombat()});renderSpellButton();renderRangeButton()}
...
function returnEarly(msg="You decide to return early."){
 ...
 addlog(msg);let now=Date.now(),away=Math.max(0,now-h.trip.start);
